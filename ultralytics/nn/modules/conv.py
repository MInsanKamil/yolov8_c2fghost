...
class GhostConv_Modification(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        c_ = c2 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, k, s, None, g, act=act)
        self.cv2 = Conv(c_, c_//4, 1, 1, None, c_//4, act=act)
        self.a = nn.AvgPool2d(3, 2, 1)
        self.fr = nn. FractionalMaxPool2d(3, output_ratio=(0.5, 0.5))
        self.m = nn.MaxPool2d(3, 2, 1)
        self.u = nn.Upsample(scale_factor=2, mode='nearest')

    def forward(self, x):
        """Forward propagation through a Ghost Bottleneck layer with skip connection."""
        y = self.cv1(x)
        return torch.cat((y,self.cv2(y), self.u(self.cv2(self.m(y))), self.u(self.cv2(self.fr(y))), self.u(self.cv2(self.a(y)))), 1)
...
class ChannelAttention_Pool(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Applies forward pass using activation on convolutions of the input, optionally using batch normalization."""
        return self.pool(x) * self.act(self.fc(self.pool1(self.pool(x))))
...
class Concat_Feature_Map(nn.Module):
    ...
    def __init__(self, dimension=1):
        ...
        super().__init__()
        self.d = dimension
        self.pool1 = nn.AvgPool2d(3,4,1)
        self.pool2 = nn.AvgPool2d(3,2,1)

    def forward(self, x):
        """Forward pass for the YOLOv8 mask Proto module."""
        return torch.cat((x[0], self.pool1(x[1]), self.pool2(x[2])), self.d)
