...
class Conv_Max_Pooling_Dropout_Attn(nn.Module):
    ...
    def forward(self, x):
        """Apply convolution, batch normalization and activation to input tensor."""
        if self.training:   
            x = self.act(self.bn(self.ca(self.conv(self.max_pool(self.dropout(x))))))
        else:
            x = self.act(self.bn(self.ca(self.conv(self.max_pool(x)))))
        return x

    def forward_fuse(self, x):
        """Perform transposed convolution of 2D data."""
        if self.training:   
            x = self.act(self.ca(self.conv(self.max_pool(self.dropout(x)))))
        else:
            x = self.act(self.ca(self.conv(self.max_pool(x))))
        return x
...
class Conv_Attn(nn.Module):
    ...
    def forward(self, x):
        """Apply convolution, batch normalization and activation to input tensor."""
        x = self.act((self.bn(self.ca(self.conv(x)))))
        return x

    def forward_fuse(self, x):
        """Perform transposed convolution of 2D data."""
        x = self.act(self.ca(self.conv(x)))
        return x
